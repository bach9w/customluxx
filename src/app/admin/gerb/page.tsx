...
export default function Dashboard() {
	...
	const create = useMutation(api.gerbs.createGerb);
	function onEvent() {
		create({
								...
								<Card className="min-h-[150px] max-h-[350px] min-w-[150px] flex items-center justify-center flex-col gap-2">
									<CardHeader className="w-full flex items-center justify-center ">
										<div className="gap-2 space-x-2">
											<AlertDelete id={gerb._id} />
											<AlertEdit id={gerb._id} />
										</div>
										<CardTitle>
											<span className="text-xl font-semibold">
												<Badge className="text-[27px]">{gerb.name}</Badge>
											</span>
										</CardTitle>
									</CardHeader>

									<ConvexImage imageId={gerb.image} />
									{gerb.status === true ? "Активен" : "Неактивен"}
								</Card>
							</div>
						))}
					</div>
